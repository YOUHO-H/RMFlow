...
try:
    ...
    raise ImportError(
        "This file requires Hugging Face diffusers. Install with `pip install diffusers accelerate transformers`."
    ) from e
...
class UNetModel(nn.Module):
    ...
    def forward(self, x, timesteps, extra=None):
        ...
        h = x
        if extra is not None and "concat_conditioning" in extra:
...
class MFUNetModel(nn.Module):
    ...
    def forward(self, x, timesteps_r, timesteps, extra=None):
        """
        Apply the model to an input batch.
        :param x: an [N x C x ...] Tensor of inputs.
        :param timesteps: a 1-D batch of timesteps.
        ...
        hs = []
        emb = self.time_embed(timestep_embedding(timesteps, self.model_channels).to(x))
        emb_r = self.time_embed(timestep_embedding(timesteps_r, self.model_channels).to(x))
        emb = emb + emb_r
...
def timestep_embedding(timesteps, dim, max_period=10000):
    """
    Standard sinusoidal embedding: [N] -> [N, dim]
    """
...
def base2_fourier_features(x: torch.Tensor, start: int = 6, stop: int = 8, step: int = 1):
    """
    Base-2 Fourier features on channels; x: [N, C, H, W] -> concat sin/cos(2^k * x)
    """
    spatial = x.shape[2:]
    x_flat = x.view(x.shape[0], x.shape[1], -1)  # [N, C, S]
    feats = []
    for k in range(start, stop, step):
        f = 2.0 ** k
        feats += [torch.sin(x_flat * f), torch.cos(x_flat * f)]
    z = torch.cat(feats, dim=1)
    return z.view(x.shape[0], z.shape[1], *spatial)


# -----------------------------
# Main Model (Diffusers-backed)
# -----------------------------

class RMFlowUNet(nn.Module):
    """
    A U-Net backbone for latent-space modeling that reuses blocks from a pretrained
    Stable Diffusion U-Net (Diffusers) 
    """

    def __init__(
        self,
        pretrained_repo_or_path: str = "runwayml/stable-diffusion-v1-5",
        device: str = "cuda",
        keep_config: Optional[Dict] = None,
        with_fourier_features: bool = False,
        ...
        # ---- Load UNet with from_pretrained ----
        unet: UNet2DConditionModel = UNet2DConditionModel.from_pretrained(
            pretrained_repo_or_path, subfolder="unet"
        )
        unet.to(device)

        base_c = unet.config.block_out_channels[0] 
        self.model_channels = model_channels or base_c

        if keep_config is None:
            keep_config = {
                "keep_down_idx": list(range(len(unet.down_blocks) - 1)),  
                "keep_up_idx":   list(range(len(unet.up_blocks) - 1)),    
                "keep_mid": True,
            }


        self.conv_in = unet.conv_in
        self.down_blocks = nn.ModuleList([unet.down_blocks[i] for i in keep_config["keep_down_idx"]])
        self.mid_block = unet.mid_block if keep_config.get("keep_mid", True) else nn.Identity()
        self.up_blocks = nn.ModuleList([unet.up_blocks[i] for i in keep_config["keep_up_idx"]])
        ...
        self.time_embed = nn.Sequential(
            ...
            nn.SiLU(),
            nn.Linear(time_embed_dim, self.model_channels),
        )


        self.label_emb = nn.Embedding(num_classes + 1, self.model_channels) if num_classes is not None else nn.Identity()
        self._device = device

    # -----------------------------
    # Forward
    # -----------------------------
    def forward(self, x, timesteps_r, timesteps, extra: Optional[Dict] = None):
        """
        :param x: [N, 4, 32, 32] latent (for 256x256 images with SD VAE f=8)
        :param timesteps: [N] tensor of main time steps
        :param timesteps_r: [N] tensor of auxiliary time steps (your r)
        :param extra: dict; expected keys (optional)
            - "encoder_hidden_states": [N, S, D] text/context embeddings for cross-attn
            - "label": [N] integer labels (class-cond)
            - "concat_conditioning": extra channels to concatenate to x (rare; avoid with SD UNet)
        :return: [N, 4, 32, 32]
        """
        if extra is None:
            extra = {}
        ...
        emb_t = self.time_embed(timestep_embedding(timesteps, self.model_channels).to(x))
        emb_r = self.time_embed(timestep_embedding(timesteps_r, self.model_channels).to(x))
        emb = emb_t + emb_r
        if self.ignore_time:
            emb = emb * 0.0

        if (self.num_classes is not None) and ("label" not in extra):
            extra["label"] = torch.full(
                (x.size(0),), self.num_classes, dtype=torch.long, device=x.device
            )
        if (self.num_classes is not None) and ("label" in extra):
            y = extra["label"]
            assert y.shape == x.shape[:1], f"Labels have shape {y.shape}, input batch is {x.shape}"
            emb = emb + self.label_emb(y)

        encoder_hidden_states = extra.get("encoder_hidden_states", None)

        if "concat_conditioning" in extra:
            x = torch.cat([x, extra["concat_conditioning"]], dim=1)
        ...
        for down in self.down_blocks:
            ...
            down_block_res_samples += list(res_samples)

        if isinstance(self.mid_block, nn.Identity):
            h_mid = h
        else:
            h_mid = self.mid_block(h, emb, encoder_hidden_states=encoder_hidden_states)

        for up in self.up_blocks:
            h_mid = up(
                hidden_states=h_mid,
                temb=emb,
                res_hidden_states_tuple=tuple(down_block_res_samples),
                encoder_hidden_states=encoder_hidden_states,
            )

        # 5) Out
        h_mid = self.conv_norm_out(h_mid)
        h_mid = self.conv_act(h_mid)
        result = self.conv_out(h_mid)  # [N, 4, 32, 32]
